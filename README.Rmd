...
**The stock-recruitment parameters output from `bootstrapSR()` are NOT estimated with the log-normal bias correction (so use `rlnormar1(bias.correct=FALSE)`).** 
...
Below are additional details for users of the `srrTMB()` function, which is internally called by `bootstrapSR()`. 
